...
Soit $X$, la variable aléatoire représentant le nombre d'accidents par assuré. $X$ est une variables aléatoire discrète et il est admis que l'occurence des sinistres $X$ suit une loi de Poisson (notée $\mathscr{P_{\lambda}}$) pour laquelle on recherche le paramètre inconnu $\lambda >0$.

Le modèle d'échantillonage de nos données est le suivant : $(\mathbb{N}, (\mathscr{P}_{\lambda})_{\lambda>0})^{n}$.
Et on a $E_\lambda(X) = \lambda = V_\lambda (X)$
...
p + ggtitle("Histogramme des sinistres") + xlab("Nombre de sinistre(s) par individus.") + ylab("Fréquence")
```

A partir de $\overline{X}_{n} = \frac{\sum_{i=0}^n x_i}{n} =$ `r mean(d1$sinistre)`, on a $n \overline{X}_{n} =$ `r dim(d1)[1]*mean(d1$sinistre)`.
...
<!--page 72 du cours, la prof nous a parlé de ça quand on lui a posé la question-->
La variable aléatoire $X$ suit une loi de Poisson de paramètre $\lambda$ et notre échantillon est de grande taille. Avec la page 72 du cours, on donne donc les régions critiques par excès en utilisant les fractiles de lois normales et la fonction pivotale asymptotique suivante.

\[\frac{\sqrt{n}(\overline{X}_n - \lambda)}{\sqrt{\overline{X}_n}} \leadsto N(0,1)\]

### choix2

Selon une estimation des intervalles de confiances sur le modèle d'échantillonngage de lois de Poisson de paramètre $\lambda$ (page 45 du cours).
Comme le modèle est discret, à distance finie, on peut calculer des intervalles de confiances par excès en utilisant la relation : 

\[ F_{n\overline{X}_n} (k) = P (\{Y_n > 2n \lambda\}) \text{, avec } Y_n \leadsto \chi^2_{2(k+1)]} \]
