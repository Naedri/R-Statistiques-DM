...
La statistique de test associée à cet échantillon est $n \overline{X}_n$, qui représente le nombre total d'observations. Et on a $n \overline{X}_n \leadsto n \lambda$.
...
A partir de $\overline{X}_{n} = \frac{\sum_{i=0}^n x_i}{n} =$ `r round(x=mean(d1$sinistre), digits=2)`, on a $n \overline{X}_{n} =$ `r dim(d1)[1]*mean(d1$sinistre)`.

## Intervalle de confiance ($IC$) unilatéral pour $\lambda$

### choix1

La variable aléatoire $X$ suit une loi de Poisson de paramètre $\lambda$ et notre échantillon est de grande taille. Avec le livret du module 2 du cours (page 72), on donne donc les régions critiques par excès en utilisant les fractiles de lois normales et la fonction pivotale asymptotique suivante.

\[\frac{\sqrt{n}(\overline{X}_n - \lambda)}{\sqrt{\overline{X}_n}} \xrightarrow{\mathit{CL}} N(0,1)\]

### choix2

Pour estimer $\lambda$, on peut utiliser, à distance finie, la fonction pivotale $n \overline{X}_n$ suivant une loi de Poisson de paramètre $n \lambda$.

Avec le livret du module 2 du cours (page 45), on trouve que selon une estimation des intervalles de confiances sur le modèle d'échantillonngage de lois de Poisson de paramètre $\lambda$ se basant sur la relation avec les lois du $\chi^2$, on peut calculer des intervalles de confiances par excès en utilisant la relation : 
