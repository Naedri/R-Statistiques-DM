...
\[( \mathbb{R}^n, \otimes_{i=1}^{n} N(\theta x_{i}^{2}, a^2)_{(\theta, a) \in \mathbb{R}^{2} \times ]0;+\infty[})\]
...
\[( \mathbb{R}^n, \otimes_{i=1}^{n} N(\theta_0 + \theta_1 x'_{i}, \sigma^2)_{(\theta_0, \theta_1, \sigma) \in \mathbb{R}^{2} \times ]0;+\infty[})\]
...
\[\hat{\theta}_n = \frac{\sum_{i=1}^{n} ( x_i^2 - \overline{{x_n}^2} ) (Y_{x_i} - Y_{\underline{X}_n})}{n S_{\underline{X}_n}^2}\]
...
```{r 3_hat_theta, echo=FALSE, results='hide'}
hatteta <- "TODO"
#hatteta <- round(hatteta, digits = 3)
```

Après application numérique, on calcule ainsi : $\hat{\theta}_n =$ `r hatteta`.
...
La borne Cramér-Rao exprime une borne inférieure sur la variance d'un estimateur sans biais, basée sur l'information de Fisher ($I$) (selon [stats.stackexchange.com](https://stats.stackexchange.com/questions/461035/cram%c3%a9r-rao-lower-bound-with-an-uncertain-observable)).
...
```{r 3_cramer_rao, echo=FALSE, results='hide'}
cramerrao <- "TODO"
#cramerrao <- round(cramerrao, digits = 3)
```

Après application numérique, on calcule ainsi : $I_{\underline{X}_n}(\theta)^{-1} =$ `r cramerrao`.
...
```{r 3_ic_theta, echo=FALSE, results='hide'}
ICGtheta <- "TODO"
#ICGtheta <- round(ICGtheta, digits = 3)
ICDtheta <- "TODO"
#ICDtheta <- round(ICDtheta, digits = 3)
```

Après application numérique, on trouve : $IC^{1-\alpha}_{Y_{\underline{X}_{n}}}(\theta) = [$ `r ICGtheta` $;$ `r ICDtheta` $]$.
